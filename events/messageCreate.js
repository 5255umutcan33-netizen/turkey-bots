...
// 🚨 TESSERACT'I ANA DÖNGÜDEN KOPARAN FONKSİYON 🚨
async function processImageOcr(buffer) {
    try {
        const { data: { text } } = await Tesseract.recognize(buffer, 'eng', { logger: () => {} });
        return text.toLowerCase().replace(/[^a-z0-9]/g, '');
    } catch (err) {
        console.error("OCR Okuma Hatası:", err);
        return "";
    }
}
...
module.exports = {
    ...
    async execute(message, client) {
        ...
        // --- BÖLÜM 1: RYPHERA GUARD (GÜVENLİK KALKANI) ---
        ...
        if (!message.member.permissions.has('Administrator')) {
            ...
            if (linkRegex.test(message.content)) {
                ...
                const warnLink = await message.channel.send(`🛡️ **Ryphera Guard:** <@${message.author.id}>, bu sunucuda reklam yapmak kesinlikle yasaktır!`);
            ...
            if (message.mentions.users.size > 5) {
                if (message.deletable) await message.delete().catch(() => {});
                await message.member.timeout(10 * 60 * 1000, 'Ryphera: Çoklu etiket spamı.').catch(() => {});
                return message.channel.send(`🛡️ **Ryphera Guard:** <@${message.author.id}>, çok fazla kişiyi etiketlediğin için **10 dakika** susturuldun!`);
            }
            ...
            if (protectedChannels.includes(message.channel.id)) {
                ...
                if (message.mentions.everyone) {
                    if (message.deletable) await message.delete().catch(() => {});
                    await message.member.timeout(timeoutDuration, 'Ryphera: İzinsiz everyone kullanımı.').catch(() => {});
                    const m = await message.channel.send(`🛡️ <@${message.author.id}>, Ryphera Guard kalkanına çarptın! \`@everyone\` yasak olduğu için **5 dakika** susturuldun.`);
                ...
                if (recentMessages.length >= 5) {
                    if (message.deletable) await message.delete().catch(() => {});
                    await message.member.timeout(timeoutDuration, 'Ryphera: Spam/Flood yapıldı.').catch(() => {});
                    spamMap.delete(message.author.id); 
                    
                    const m = await message.channel.send(`🛡️ <@${message.author.id}>, Ryphera Guard kalkanına çarptın! \`Spam\` yaptığın için **5 dakika** susturuldun.`);
                    setTimeout(() => m.delete().catch(() => {}), 5000);
                    return;
                }
            }
        ...
        if (isTR || isEN) {
            ...
            // 🚨 BOTU KİTLEMEYEN ARKA PLAN İŞLEMİ 🚨
            setImmediate(async () => {
                try {
                    ...
                    const cleanedText = await processImageOcr(buffer);
                    ...
                    if (cleanedText.includes(target)) {
                        ...
                        if (logChannel) {
                            const embed = new EmbedBuilder()
                                ...
                                .setTimestamp()
                                .setFooter({ text: 'Ryphera OS Log System' });
            ...
            return; // OCR işlemi arka planda akarken biz asıl kodun sonlanmasına izin veriyoruz
        }
